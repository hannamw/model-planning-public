...
for model in models[-1:]:
    ...
    for idx, row in tqdm(metadata.iterrows()):
        ...
        if False and selected_nodes is not None and len(selected_nodes) > 0:
...
print("fetching features from set")
# get all features at once
get_features_with_cache(list(feature_set), feature_info_cache, model)
print("feature fetched")
#%%
# Process each example based on metadata
for idx, row in tqdm(metadata.iterrows()):
    correct_article = row['Article']
    ...
    last_word_features = selected_features[selected_features[:, 1] == graph.n_pos - 1]
    selected_nodes = [(layer, pos, feature) for layer, pos, feature in last_word_features.tolist() 
                        if is_word_feature(layer, feature, english_noun)]
...
print(f"    - {examples_without_nodes} examples without important nodes (original probabilities recorded)")
# %%
is_word_feature(36, 102532, 'apple')
# %%
correct_article = 'la'
noun = 'manzana'
english_noun = 'apple'
...
selected_nodes = [(layer, pos, feature) for layer, pos, feature in last_word_features.tolist() 
                    if is_word_feature(layer, feature, english_noun)]
# %%
selected_features_set = set(tuple(x) for x in selected_features.tolist())
last_word_features_set = set(tuple(x) for x in last_word_features.tolist())
# %%
(36, 33, 102532) in selected_features_set
#%%
(36, 33, 102532) in last_word_features_set
#%%
# %%
graph.n_pos

# %%
def pinned_ids_string(features):
    return '&pinnedIds=' + '%2C'.join([f'{layer}_{feat}_{pos}' for layer, pos, feat in features])
# %%
pinned_ids_string(selected_nodes)
# %%
correct_article = 'la'
noun = 'manzana'
english_noun = 'apple'
...
if selected_nodes is not None and len(selected_nodes) > 0:
    ...
    #sn = torch.tensor(selected_nodes)
    #print(sn.max(0).values)
    zero_interventions = [(*feat, -2 * original_acts[tuple(feat)]) for feat in selected_nodes]
    multiply_interventions = [(*feat, 5 * original_acts[tuple(feat)]) for feat in selected_nodes]
