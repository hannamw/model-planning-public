...
for model_name, model_config in model_names_and_configs:
    ...
    a_token = model.tokenizer(' a').input_ids[0]
    an_token = model.tokenizer(' an').input_ids[0]

    a_minus_an = model.W_U[:, a_token] - model.W_U[:, an_token]
    an_minus_a = model.W_U[:, an_token] - model.W_U[:, a_token]
    ...
    for sentence, continuation, name in examples:
        ...
        if continuation == ' a':
            quantity_to_attribute = [('a - an', 1.0, a_minus_an)]
        elif continuation == ' an':
            quantity_to_attribute = [('an - a', 1.0, an_minus_a)]
        else:
