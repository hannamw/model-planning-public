...
for model in models:
    ...
    ax1.plot(x_range, results['a_correct']['direct'].cpu().numpy(), 'g-', label='Direct', linewidth=2)
    ax1.plot(x_range, results['a_correct']['indirect'].cpu().numpy(), 'r-', label='Indirect', linewidth=2)
    ...
    ax2.plot(x_range, results['a_incorrect']['direct'].cpu().numpy(), 'g-', label='Direct', linewidth=2)
    ax2.plot(x_range, results['a_incorrect']['indirect'].cpu().numpy(), 'r-', label='Indirect', linewidth=2)
    ...
    ax3.plot(x_range, results['an_correct']['direct'].cpu().numpy(), 'g-', label='Direct', linewidth=2)
    ax3.plot(x_range, results['an_correct']['indirect'].cpu().numpy(), 'r-', label='Indirect', linewidth=2)
    ...
    ax4.plot(x_range, results['an_incorrect']['direct'].cpu().numpy(), 'g-', label='Direct', linewidth=2)
    ax4.plot(x_range, results['an_incorrect']['indirect'].cpu().numpy(), 'r-', label='Indirect', linewidth=2)
...
def create_aggregated_plot(article_type, all_model_results, models, xlim=None, model_sizes=None, to_plot=['direct', 'indirect']):
    ...
    line_styles = {
        ('direct', True): '-',      # solid
        ('direct', False): '--',    # dashed
        ('indirect', True): ':',  # dotted
        ('indirect', False): '-.' # dash-dot
    }
    ...
    for model_idx, model in enumerate(models):
        ...
        # Plot direct and indirect for both correct and incorrect cases
...
def create_max_influence_scatter_plots(all_model_results, models):
    ...
    ax1.set_title('"a" Examples - Direct Node Influence')
    ...
    ax2.set_title('"an" Examples - Direct Node Influence')
...
def create_mean_max_influence_scatter_plots(all_model_results, models):
    ...
    ax1.set_title('"a" Examples - Mean Direct Node Influence')
    ...
    ax2.set_title('"an" Examples - Mean Direct Node Influence')
...
def create_combined_mean_influence_bar_plot(all_model_results, models):
    ...
    ax.set_title('Mean Proportion of Direct Influence from Selected Nodes by Model', fontsize=24)
    ax.set_xticks(x_pos)
    ...
    plt.tight_layout()
    plt.savefig('results/directness_influence.pdf')
    plt.show()
