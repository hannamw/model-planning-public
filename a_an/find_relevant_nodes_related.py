...
def load_profession_terms(csv_path: str = 'data/professions_dataset_related_terms_refined.csv') -> dict:
    """
    Load profession and related terms from CSV file.
    
    Args:
        csv_path: Path to CSV file containing professions and related terms
        
    Returns:
        Dictionary mapping profession to list of related terms
    """
    df = pd.read_csv(csv_path)
    profession_terms = {}
    for _, row in df.iterrows():
        profession = row['Profession']
        related_terms = [term.strip() for term in row[['related_term1', 'related_term2', 'related_term3', 'related_term4', 'related_term5']].values]
        profession_terms[profession] = related_terms
    return profession_terms
...
def process_model_relevant_nodes(
    ...
    profession_terms = load_profession_terms()
    ...
    for _, row in tqdm(metadata.iterrows(), desc=f"Processing {model}"):
        ...
        related_terms = profession_terms.get(profession, [])
        ...
        for layer, pos, feature_idx in active_features_list:
            ...
            # Count profession mentions
            profession_count = sum(1 for text in texts if profession.lower() in text.lower())
            
            # Count related terms
            term_counts = count_terms_in_texts(texts, related_terms)
            ...
            feature_counts[feature_key] = {
                ...
                'total_count': profession_count + related_terms_count,
