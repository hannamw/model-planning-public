...
if selected_nodes is not None and len(selected_nodes) > 0:
    ...
    for inter in multiply_interventions:
        print(inter)
        freeze_interventions = [(*feat, original_acts[feat]) for feat in [(27, 16, 80153), (27, 16, 141224), (26, 16, 75185)]]
        if direct:
            logits_zeros, acts_zeros = replacement_model.feature_intervention(s, interventions=zero_interventions, constrained_layers=range(1, replacement_model.cfg.n_layers))
            logits_multiply, acts_multiply = replacement_model.feature_intervention(s, interventions=multiply_interventions[5:6], constrained_layers=range(1, replacement_model.cfg.n_layers))
        else:
            logits_zeros, acts_zeros = replacement_model.feature_intervention(s, interventions=zero_interventions)
            logits_multiply, acts_multiply = replacement_model.feature_intervention(s, interventions=[inter])#multiply_interventions[5:6])# + freeze_interventions)
