...
old_transcoder_names = {models[i]: old_name for i, old_name in 
    enumerate(['qwen3-0.6b-relu-lowl0', 'qwen3-1.7b-relu-lowl0', 'qwen3-4b-relu', 'qwen3-8b-relu', 'qwen3-14b-relu-lowl0'])}
...
for model in models:
    ...
    for layer in range(replacement_model.cfg.n_layers):
        top_bottom_by_layer[layer] = load_top_logits(old_transcoder_names[model], layer)
    print("done")
    ...
    for idx, row in tqdm(metadata.iterrows()):
        ...
        original_probs = torch.softmax(original_logits.squeeze(0)[-1, :], dim=-1)
        ...
        candidate_nodes = graph.active_features[graph.active_features[:, 1] == n_pos - 1] if LAST_ONLY else graph.active_features
        ...
        if selected_nodes is not None and len(selected_nodes) > 0:
            ...
            freeze_a_an = [(*feat, original_acts[tuple(feat)]) for feat in a_an_feature_indices]
            ...
            logits_zeros, _ = replacement_model.feature_intervention(s, interventions=zero_interventions + freeze_a_an, return_activations=False)
            logits_multiply, _ = replacement_model.feature_intervention(s, interventions=multiply_interventions + freeze_a_an, return_activations=False)
