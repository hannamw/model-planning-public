...
LAST_ONLY = True
INTERVENTION_RESULTS_DIR = Path('results/interventions_random_last') if LAST_ONLY else Path('results/interventions_random')  # Directory for path length results
...
for model in logit_lens_to_transcoders.keys():
    top_bottom_by_layer = {}
    ...
    for idx, row in tqdm(metadata.iterrows()):
        ...
        if selected_nodes:
            all_features = graph.active_features[graph.selected_features]
            random_idxs = torch.randperm(len(all_features))[:len(selected_nodes)]
            randomly_selected_nodes = all_features[random_idxs].tolist()
        else:
            randomly_selected_nodes = None
        
        n_pos = graph.n_pos 
        if LAST_ONLY and selected_nodes:
            selected_nodes = [(layer, pos, idx) for layer, pos, idx in selected_nodes if pos == n_pos - 1]
            last_features = all_features[all_features[:, 1] == n_pos - 1]
            random_idxs = torch.randperm(len(last_features))[:len(selected_nodes)]
            randomly_selected_nodes = last_features[random_idxs]
        ...
        if randomly_selected_nodes is not None and len(randomly_selected_nodes) > 0:
            #print(original_acts.size())
            #sn = torch.tensor(selected_nodes)
            #print(sn.max(0).values)
            zero_interventions = [(*feat, 0) for feat in randomly_selected_nodes]
            multiply_interventions = [(*feat, 5 * original_acts[tuple(feat)]) for feat in randomly_selected_nodes]
