...
def threshold_relevance_function(texts: list[str], target_word: str, threshold: int = 5) -> tuple[bool, dict]:
    """
    Simple threshold-based counting using substring matching.
    
    Args:
        texts: List of text strings to analyze
        target_word: The word/phrase to check relevance against
        threshold: Minimum number of matching texts for relevance
        
    Returns:
        Tuple of (is_relevant, metadata_dict)
    """
    count = sum(1 for text in texts if target_word.lower() in text.lower())
    return count >= threshold, {'count': count}

def count_target_word(texts: list[str], target_word: str) -> int:
    """
    Count occurrences of target word in texts using substring matching.
    
    Args:
        texts: List of text strings to analyze
        target_word: The word/phrase to count
        
    Returns:
        Count of texts containing the target word
    """
    return sum(1 for text in texts if target_word.lower() in text.lower())
...
def process_model_relevant_nodes(
    ...
    save_dir: str = 'results/relevant_nodes',
    overwrite: bool = False
):
    """
    Process a single model to count profession mentions for all active features.
    ...
    for _, row in tqdm(metadata.iterrows(), desc=f"Processing {model}"):
        ...
        # Count profession mentions for each active feature
        feature_counts = {}
        
        # For each active (layer, pos, feature_idx) tuple, get the count
        for layer, pos, feature_idx in active_features_list:
            texts = feature_texts.get((layer, feature_idx), [])
            count = count_target_word(texts, profession)
            feature_counts[(layer, pos, feature_idx)] = count
        ...
        save_data = {
            ...
            'node_threshold': node_threshold,
            'edge_threshold': edge_threshold,
            'feature_counts': {f"{layer}_{pos}_{feature_idx}": count for (layer, pos, feature_idx), count in 
