...
for model_name in models:
    ...
    for idx, row in metadata.iterrows():
        ...
        if not valid_rows.empty:
            ...
            downweight_interventions = [(layer, slice(-1, None), feature, -3 * act) 
                                    for layer, feature, act in id_to_features_acts[id]]
            new_word_interventions = [(layer, slice(-1, None), feature, 7 * act) 
    ...
    Path('results/rhyme_intervention_second_line').mkdir(parents=True, exist_ok=True)
    metadata.to_csv(f'results/rhyme_intervention_second_line/{model_name}.csv')
    del model
