...
# All Qwen3 models to iterate over
QWEN3_MODELS = [
    "Qwen/Qwen3-0.6B",
    "Qwen/Qwen3-1.7B", 
    "Qwen/Qwen3-4B",
...
def get_or_create_rhyme_group(word: str, rhyme_groups: Dict[str, Set[str]], rhyme_cache: Dict[str, Set[str]]) -> str:
    ...
    rhyming_words = fetch_rhymes(word, rhyme_cache)
    # Include the word itself in its rhyme group
    rhyme_group = {word} | rhyming_words
    rhyme_groups[word] = rhyme_group
...
def generate_single_completion(
    ...
    rhyme_group = get_or_create_rhyme_group(last1, rhyme_groups, rhyme_cache)
    ...
    rhyme_success = bool(last1 and last2 and last2 in fetch_rhymes(last1, rhyme_cache) and last1 != last2)
...
def main(argv: Sequence[str] | None = None) -> None:  # noqa: D401
    parser = argparse.ArgumentParser(description="Generate greedy rhyming couplets via HuggingFace Transformers for all Qwen3 models.")
    parser.add_argument("--input", type=Path, required=True, help="Text file with first lines (one per line).")
    parser.add_argument("--output-dir", type=Path, required=True, help="Output directory to save CSV results.")
    ...
    # Iterate over all Qwen3 models
    for model_id in QWEN3_MODELS:
        _echo(f"\n{'='*60}")
        _echo(f"Processing model: {model_id}")
        _echo(f"{'='*60}")
        
        # ------------------------------------------------------------------
        # Load model and tokenizer
        # ------------------------------------------------------------------
        _echo(f"Loading model {model_id} ...")
        try:
            ...
            _echo(f"Failed to load model {model_id}: {exc}")
            continue

        # ------------------------------------------------------------------
        # Generate completions for this model
        # ------------------------------------------------------------------
        rhyme_cache: Dict[str, Set[str]] = {}
        rhyme_groups: Dict[str, Set[str]] = {}
        results: List[dict] = []
        ...
        try:
            for idx, fl in enumerate(first_lines, start=1):
                _echo(f"[{idx}/{len(first_lines)}] Generating 100 samples for input …")
        ...
        if interrupted:
            _echo(f"Partial results ({len(results)} samples) written to {output_path}.")
            break
        else:
            _echo(f"Done ✓  Wrote {len(results)} samples to {output_path}.")
        ...
        _echo(f"Cleaned up model {model_id}")
    
    if not interrupted:
        _echo(f"\n{'='*60}")
        _echo("All models processed successfully!")
        _echo(f"{'='*60}")
