...
for model_name in models:
    ...
    for idx, row in metadata.iterrows():
        ...
        all_input_tokens = model.tokenizer.decode(graph.input_tokens) + ' ' + second_last_word
        print(all_input_tokens)
        _, acts = model.get_activations(all_input_tokens, sparse=True)
        acts_indices = acts.indices().T
        last_word = input_tokens.index(model.tokenizer.eos_token)
        last_word_features = acts_indices[acts_indices[:, 1] == acts.size(1) - 1]
    ...
    for idx, row in metadata.iterrows():
        ...
        # stop generation
        stop_interventions = [(layer, -1, feature, 5 * acts[layer, graph.n_pos, feature]) 
        ...
        # continue generation
        continue_interventions = [(layer, slice(-1, None), feature, -5 * acts[layer, graph.n_pos, feature]) for layer, feature in eol_features.keys()]
    ...
    metadata.to_csv(f'results/eol_intervention_actual_last/{model_name}.csv')