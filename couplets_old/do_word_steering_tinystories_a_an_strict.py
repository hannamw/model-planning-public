...
def term_in_logits(term:str, top: list[str], bottom: list[str], use_bottom=True, substring_ok=False, k=5):
...
for model_name in models:
    ...
    for article, profession in zip(metadata['correct_articles'], metadata['professions']):
        ...
        tokenized_profession = model.tokenizer.decode(model.tokenizer(' ' + profession).input_ids[0]).strip()
        profession_to_token[profession] = tokenized_profession
        print(profession, tokenized_profession)
    ...
    for article, profession in zip(metadata['correct_articles'], metadata['professions']):
        ...
        features_with_acts = [(layer, feat, acts[layer, pos, feat].item()) 
                                ...
                                if  is_word_feature(layer, feat, profession) 
                                    or is_say_x_feature(layer, feat, profession)]# profession_to_token[profession])]
    ...
    top10_professions = professions_sorted[:10]

    print(top10_professions)
    ...
    results_dir = Path("results/word_steering_tinystories_a_an_strict")
