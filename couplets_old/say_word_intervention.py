...
for model_name in models:
    ...
    # First pass: collect all features for batch loading
    ...
    for idx, row in metadata.iterrows():
        ...
        near_word_features = {k:v for k,v in last_word_feature_infos.items() if is_near_word_feature(*k, second_last_word)}
        word_features = {k:v for k,v in last_word_feature_infos.items() if is_word_feature(*k, second_last_word)}
        say_word_features = {k:v for k,v in last_word_feature_infos.items() if is_say_x_feature(*k, second_last_word)}
        
        all_word_features = {k for k in [*near_word_features.keys(), *word_features.keys(), *say_word_features.keys()]}
        id_to_features_acts[f"{idx}-{second_last_word}"] = [(layer, feat, acts[layer, graph.n_pos - 1, feat]) 
                                                            for layer, feat in all_word_features]
    ...
    metadata['original_top5_tokens'] = ''
    metadata['original_top5_probs'] = ''
    metadata['intervention_top5_tokens'] = ''
    metadata['intervention_top5_probs'] = ''
    ...
    for idx, row in metadata.iterrows():
        ...
        graph = Graph.from_pt(graph_dir / f"{idx}-{second_last_word}.pt")
        input_tokens = model.tokenizer.convert_ids_to_tokens(graph.input_tokens)
        input_text = model.tokenizer.decode(graph.input_tokens)

        # record normal top-5
        normal_logits = model(input_text).squeeze(0)
        normal_probs = torch.softmax(normal_logits, dim=-1)
        normal_top5_probs, normal_top5_indices = torch.topk(normal_probs[-1], k=5)
        normal_top5_tokens = [model.tokenizer.decode(token_idx) for token_idx in normal_top5_indices]
        metadata.at[idx, 'original_top5_tokens'] = str(normal_top5_tokens)
        metadata.at[idx, 'original_top5_probs'] = str(normal_top5_probs.tolist())
        ...
        if not valid_rows.empty:
            ...
            downweight_interventions = [(layer, graph.n_pos - 1, feature, -3 * act) 
                                    for layer, feature, act in id_to_features_acts[id]]
            new_word_interventions = [(layer, graph.n_pos - 1, feature, 5 * act) 
                                    for layer, feature, act in id_to_features_acts[chosen_id]]

            new_logits, _ = model.feature_intervention(input_text, 
                                                       interventions=downweight_interventions + new_word_interventions,
                                                       return_activations=False)
            
            # record new top-5
            new_probs = torch.softmax(new_logits, dim=-1).squeeze(0)
            new_top5_probs, new_top5_indices = torch.topk(new_probs[-1], k=5)
            new_top5_tokens = [model.tokenizer.decode(token_idx) for token_idx in new_top5_indices]
            metadata.at[idx, 'intervention_top5_tokens'] = str(new_top5_tokens)
            metadata.at[idx, 'intervention_top5_probs'] = str(new_top5_probs.tolist())
            ...
            metadata.at[idx, 'intervention_top5_tokens'] = 'DUMMY_TOP5_TOKENS'
            metadata.at[idx, 'intervention_top5_probs'] = 'DUMMY_TOP5_PROBS'

    # Ensure output directory exists
    Path('results/say_word_intervention').mkdir(parents=True, exist_ok=True)
