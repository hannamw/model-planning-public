...
ax1.set_ylabel('Change in p(correct)')
ax1.set_title('Effect of Direct Zero Ablations on p(correct)')
ax1.set_xticks(range(len(models)))
ax1.axhline(y=0, color='black', linestyle='--', alpha=0.5)
ax1.grid(True, alpha=0.3)
...
ax2.set_ylabel('Change in p(correct)')
ax2.set_title('Effect of Direct Multiplying Interventions on p(correct)')
ax2.set_xticks(range(len(models)))
ax2.set_xticklabels(models, rotation=0)
ax2.axhline(y=0, color='black', linestyle='--', alpha=0.5)
ax2.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig('intervention_effects_direct_line.pdf', bbox_inches='tight')
plt.show()
# %%
