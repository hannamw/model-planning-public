...
LAST_ONLY = True
INTERVENTION_RESULTS_DIR = Path('results/a_an_intervention_selected_last') if LAST_ONLY else Path('results/a_an_intervention_selected')  # Directory for path length results
...
for model in models:
    ...
    metadata['say_a_an_node_zeroed_activation_diff_good'] = None
    metadata['say_a_an_node_multiplied_activation_diff_good'] = None
    metadata['say_a_an_node_zeroed_activation_diff_bad'] = None
    metadata['say_a_an_node_multiplied_activation_diff_bad'] = None
    metadata['n_good_a_an_features'] = None
    metadata['n_bad_a_an_features'] = None
    ...
    for idx, row in tqdm(metadata.iterrows()):
        ...
        diff_graph = Graph.from_pt(str(diff_graph_file))
        ...
        is_a_an_vector = torch.tensor([a_an_in_logits_count(top_bottom_by_layer[layer][0][feat], top_bottom_by_layer[layer][1][feat], count=3) 
            for layer, pos, feat in diff_graph.active_features[diff_graph.selected_features].tolist()])
        selected_feature_scores = diff_graph.adjacency_matrix[-1, :len(diff_graph.selected_features)]
                                   
        if is_a_an_vector.any():
            a_an_feature_indices = diff_graph.active_features[diff_graph.selected_features][is_a_an_vector]
            a_an_scores = selected_feature_scores[is_a_an_vector]
            original_a_an_activations = original_acts[a_an_feature_indices[:, 0], a_an_feature_indices[:, 1], a_an_feature_indices[:, 2]]

            n_good_a_an_features = (a_an_scores > 0).sum().item()
            n_bad_a_an_features = (a_an_scores < 0).sum().item()
        else:
            a_an_feature_indices = []
            original_a_an_activations = []
            n_good_a_an_features = 0
            n_bad_a_an_features = 0
        ...
        if selected_nodes is not None and len(selected_nodes) > 0:
            ...
            if len(a_an_feature_indices):
                ...
                zeroed_a_an_feature_diffs_signed = (zeroed_a_an_feature_activations - original_a_an_activations) / original_a_an_activations
                multiplied_a_an_feature_diffs_signed = (multiplied_a_an_feature_activations - original_a_an_activations) / original_a_an_activations

                if (a_an_scores > 0).any():
                    say_a_an_node_zeroed_activation_diff_good = zeroed_a_an_feature_diffs_signed[a_an_scores > 0].mean().item()
                    say_a_an_node_multiplied_activation_diff_good = multiplied_a_an_feature_diffs_signed[a_an_scores > 0].mean().item()
                else:
                    say_a_an_node_zeroed_activation_diff_good = 0
                    say_a_an_node_multiplied_activation_diff_good = 0

                if (a_an_scores < 0).any():
                    say_a_an_node_zeroed_activation_diff_bad = zeroed_a_an_feature_diffs_signed[a_an_scores < 0].mean().item()
                    say_a_an_node_multiplied_activation_diff_bad = multiplied_a_an_feature_diffs_signed[a_an_scores < 0].mean().item()
                else:
                    say_a_an_node_zeroed_activation_diff_bad = 0
                    say_a_an_node_multiplied_activation_diff_bad = 0
        ...
        metadata.at[idx, 'say_a_an_node_zeroed_activation_diff_good'] = say_a_an_node_zeroed_activation_diff_good
        metadata.at[idx, 'say_a_an_node_multiplied_activation_diff_good'] = say_a_an_node_multiplied_activation_diff_good
        metadata.at[idx, 'say_a_an_node_zeroed_activation_diff_bad'] = say_a_an_node_zeroed_activation_diff_bad
        metadata.at[idx, 'say_a_an_node_multiplied_activation_diff_bad'] = say_a_an_node_multiplied_activation_diff_bad
        metadata.at[idx, 'n_good_a_an_features'] = n_good_a_an_features
        metadata.at[idx, 'n_bad_a_an_features'] = n_bad_a_an_features
