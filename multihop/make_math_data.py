...
counter = 0
eqs = set()
results = {k:[] for k in ['variable', 'op1', 'op2', 'n1', 'n2', 'n3', 'intermediate', 'answer', 'eq1', 'eq2', 'eqfull']}
alphabet = [chr(i) for i in range(ord('a'), ord('z')+1)]
operators = ['+', '-', '*', '//']
while counter < 10000:
    variable = random.choice(alphabet)
    ...
    eq1 = f'{variable}={n1}{op1}{n2}'
    eq2 = f'{variable}{op2}{n3}='
    eqfull = f'{eq1}. {eq2}'
    if not eqfull in eqs:
        ...
        results['eq1'].append(eq1)
        results['eq2'].append(eq2)
        results['eqfull'].append(eqfull)
        counter += 1

df = pd.DataFrame(results)
df.to_csv('pre-data/math_data.csv', index=False)
# %%
