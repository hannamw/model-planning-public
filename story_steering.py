#%%
stories = [
  "Maria discovered her grandmother's recipe box contained more",
  "The new coffee shop on Main Street served drinks that tasted exactly",
  "After twenty years of teaching, Mrs. Chen",
  "The lost dog had been wearing a collar with coordinates to a place",
  "Emma's attempt to organize her closet led to finding letters",
  "The community garden plot came with a notebook full of advice from previous",
  "On his first day of retirement,",
  "The library's suggestion box started giving",
  "When the elevator got stuck",
  "The inheritance from her great-aunt included a"
]
#%%
import os
...
upweight_bright = [(l, slice(-1, None), f, 6 * bright_acts[l,p,f]) for l,p,f in bright_features['say bright']]
#%%
...
upweight_flight = [(l, slice(-1, None), f, 2 * flight_acts[l,p,f]) for l,p,f in flight_features['fly/flight']]# + flight_features['fl']]
#%%
names_and_upweights = [("night", upweight_night), ("bright", upweight_bright), ("accountant", upweight_accountant), ("flight", upweight_flight)]
for story in stories[2:5]:
    print(model.generate(story, do_sample=False))
    for name, upweight in names_and_upweights:
        generation, _, _ = model.feature_intervention_generate(story, upweight, do_sample=False)
        print(f"{name}:", generation)
    print('-------------------------')
# %%
upweight_accountant = [(l, slice(-1, None), f, 4 * acct_acts[l,p,f]) for l,p,f in accountant_features['accounting (2)']]
generation, _, _ = model.feature_intervention_generate(stories[0], upweight_accountant, do_sample=False)
print(f"{name}:", generation)
# %%
upweight_bright = [(l, slice(-1, None), f, 5 * bright_acts[l,p,f]) for l,p,f in bright_features['say bright']]
generation, _, _ = model.feature_intervention_generate(stories[0], upweight_bright, do_sample=False)
print(f"{name}:", generation)
# %%
upweight_flight = [(l, slice(-1, None), f, 2 * flight_acts[l,p,f]) for l,p,f in flight_features['fly/flight'] + flight_features['fl']]
generation, _, _ = model.feature_intervention_generate(stories[1], upweight_flight, do_sample=False)
print(f"flight:", generation)
#%%
upweight_one = [(l, slice(-1,None), f, 3 * one_acts[l,p,f]) for l,p,f in one_features['one'] + one_features['one by one'] + one_features['X-by/for-X']]
generation, _, _ = model.feature_intervention_generate(stories[1], upweight_one, do_sample=False)
print(f"one:", generation)
# %%
def logit_lens(vector):
    return model.ln_final(vector) @ model.W_U
#%%
night_vectors = [model.transcoders.transcoders[layer].W_dec[feat] for layer, _, feat, _ in upweight_night]
# %%
night_logit_lenses = torch.stack([logit_lens(night_vector) for night_vector in night_vectors])

# %%
def print_topk_one(logits:torch.Tensor, k=5):
    probs = torch.softmax(logits, dim=-1)
    topk = torch.topk(probs, k)
    for i in range(k):
        print(model.tokenizer.decode([topk.indices[i]]), ':', topk.values[i].item())
#%%
print_topk_one(night_logit_lenses[2], k = 100)
# %%
vecs = [model.transcoders.transcoders[layer].W_dec[feat] for layer, _, feat, _ in upweight_accountant]
logit_lenses = torch.stack([logit_lens(night_vector) for night_vector in vecs])
for ll in logit_lenses:
    print_topk_one(ll, k = 100)
    print('------------------------------')

# %%
model.tokenizer(' during')
...
# %%
logit_lenses[1][74921]

# %%
night_logit_lenses[1][2337]

# %%
night_logit_lenses[1].sort(descending=True)[0][:100]
# %%
logit_lenses[1].sort(descending=True)[0][:100]
